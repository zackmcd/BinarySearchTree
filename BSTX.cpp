...
int BinarySearchTreeX<Comparable>::height(BinaryNodeX<Comparable> *t) const
{
  ...
  if (t->left)
    countLeft += height(t->left) + 1;
  
  if (t->right)
    countRight += height(t->right) + 1;
...
{ // this takes in an int in this case for main

  BinaryNodeX<Comparable> *node = root;
  BinaryNodeX<Comparable> *node2 = NULL;
 
  while( node != NULL )
    if( possibleAncestor < node->element )
      node = node->left;
    else if( node->element < possibleAncestor )
      node = node->right;
    else
    {
      node2 = node;
...
{ 
  ...
  {
    count++;
    tempL = deepestFull(t->left);
    tempR = deepestFull(t->right);
